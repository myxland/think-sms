...
class SendConfig extends Command
{
    ...
    {
        $this->setName('sms:config')->setDescription('send config to tp folder');
    }

    public function execute(Input $input, Output $output)
    {
        //获取默认配置文件
        $content = file_get_contents(Env::get('root_path') . 'vendor/' . 'myxland/think-sms/src/config.php');
